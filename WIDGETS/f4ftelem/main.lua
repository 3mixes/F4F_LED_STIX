-- Versão Funcional com Coexistência de Controlo Manual e Temporizador Automático (Interval)
...
local function drawCutBox(x, y, w, h, c, color)
  ...
  lcd.drawLine(x + w, y + h - c, x + w - c, y + h, SOLID, CUSTOM_COLOR)
...
local function refresh(w)
  ...
  local timerSeconds = getValue("timer1") or getValue("t1") or 0

  local hasTelemetry = (rqly > 0) or (rxbt > 0)
  local now = getTime()

  -- Leitura do Intervalo configurado (em segundos, convertendo para ticks de 10ms -> * 100)
  local interval_sec = (w.options and w.options.Interval) or 5
  local interval_ticks = interval_sec * 100

  -- Leitura robusta da opção SOURCE (Internal Control)
  ...
  -- Normalização para percentagem (-100 a 100)
  if control_val > 100 or control_val < -100 then
    control_val = (control_val / 1024) * 100
  end

  -- Lógica de Controlo Manual por Fim de Curso
  ...
  -- Se houve mudança manual, atualiza a referência temporal do temporizador automático
  if manual_changed then
    w.last_auto_time = now
  else
    -- Temporizador Automático (Interval)
  ...
  elseif w.indice == 3 then
    ...
    local t_min = math.floor(math.abs(timerSeconds) / 60)
    local t_sec = math.floor(math.abs(timerSeconds) % 60)
    lcd.setColor(CUSTOM_COLOR, C_LABEL)
